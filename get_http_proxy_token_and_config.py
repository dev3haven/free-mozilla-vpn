...
# -*- coding: utf-8 -*-
"""
mozvpn.py — реквизиты прокси Mozilla VPN / Firefox IP Protection.

Поток:
  1. POST /v1/account/credentials/status -> версия стретчинга (v1/v2) + clientSalt
  1a. /v1/account/login (email+authPW) -> sessionToken  [только без --session-token]
...
Использование:
  python3 mozvpn.py                                  # интерактивно
  python3 mozvpn.py --email a@b.c --password '***'
...
Переменные окружения: MOZVPN_EMAIL, MOZVPN_PASSWORD, MOZVPN_SESSION_TOKEN, MOZVPN_TOTP
...
CACHE       = os.path.join(os.path.expanduser("~"), ".config", "mozvpn", "session.json")
FASTLY_CACHE = os.path.join(os.path.expanduser("~"), ".config", "mozvpn", "fastly-cookie.json")
...
def fxa_login(email: str, password: str, totp: str = None) -> str:
    ...
    if status == 406:
        sys.exit(WAF_MESSAGE)
    if status != 200:
        errno = d.get("errno"); msg = d.get("message", d)
        if errno == 102: sys.exit("❌ Аккаунт не найден.")
        if errno == 103: sys.exit("❌ Неверный пароль (errno 103). "
                                  "Если входили через Google/Apple — сначала задайте пароль: "
                                  "accounts.firefox.com → Settings.")
        if errno == 114: sys.exit("❌ Аккаунт заблокирован (повторите позже).")
        if errno in (142, 144): sys.exit("❌ Вход с этим email запрещён / пароль не задан "
                                         "(аккаунт создан через Google/Apple). Задайте пароль: "
                                         "accounts.firefox.com → Settings.")
        sys.exit(f"❌ Ошибка входа ({status}): {msg}")
    ...
    if not verified:
        ...
        if vm in ("totp-2fa", "totp"):
            ...
            for attempt in range(3):
                raw = (totp or "").strip() or input(
                    "🔐 Код двухфакторной аутентификации (TOTP): ").strip()
                ...
                if s2 == 200:
                    if attempt == 2:
                        sys.exit("❌ Код 2FA не подошёл три раза подряд.")
                    print("⚠️  Неверный код, попробуйте ещё раз.")
                else:
                    sys.exit(f"❌ Ошибка проверки 2FA ({s2}): {d2} "
                             "(проверьте sessionToken / аккаунт)")
                totp = None
        elif vm.startswith("email"):
            sys.exit("❌ Требуется подтверждение входа по email. Войдите в браузере, "
                     "извлеките sessionToken и запустите с --session-token.")
        else:
            sys.exit(f"❌ Сессия не подтверждена (метод: {vm or 'неизвестен'}, причина: {vr}).")
...
def oauth_token(session_token: str) -> str:
    ...
    for scope in OAUTH_SCOPES:
        ...
        if status == 401 and d.get("errno") == 110:
            sys.exit("❌ sessionToken недействителен/истёк (errno 110) — "
                     "войдите в Firefox и переизвлеките его.")
        if status == 400 and d.get("errno") in (106, 125):
            sys.exit("❌ sessionToken недействителен/заблокирован — "
                     "войдите в Firefox и переизвлеките его.")
        sys.exit(f"❌ OAuth-токен не получен ({status}): {d}")
    sys.exit(f"❌ OAuth: все scope отклонены сервером (errno 114). Последняя ошибка: {last_err}")
...
def guardian_pass(oauth: str):
    ...
    Возвращает (token, until, headers_dict)."""
    ...
    if status != 200:
        ...
        if status == 403:
            sys.exit("❌ proxyPass не получен (HTTP 403, no_entitlement).\n"
        ...
        if status == 401:
            sys.exit("❌ proxyPass не получен (HTTP 401, reauth_required): "
                     "сессия/FxA-токен отклонены Guardian'ом — перелогиньтесь.")
        if status == 429:
            retry = hdrs.get("retry-after", "?")
            sys.exit(f"❌ proxyPass не получен (HTTP 429): квота исчерпана, "
                     f"повторите через {retry} с.")
        if status == 451:
            sys.exit("❌ proxyPass не получен (HTTP 451): регион недоступен.")
        sys.exit(f"❌ proxyPass не получен (HTTP {status}): {detail or 'пустой ответ'}")
    token = d.get("token")
    if not token:
        sys.exit(f"❌ В ответе Guardian нет поля 'token': {str(d)[:200]}")
...
def fetch_serverlist(firefox_version: str = DEFAULT_FIREFOX_VERSION,
    ...
    sys.exit("❌ Список серверов не загружен ни из Remote Settings (vpn-serverlist), "
...
def main():
    ...
    ap.add_argument("--totp",        default=os.environ.get("MOZVPN_TOTP"),
                    help="код 2FA (TOTP), иначе спросит интерактивно")
    ...
    email, session_token = a.email, a.session_token

    if session_token and not all(ch in "0123456789abcdefABCDEF" for ch in session_token):
        sys.exit("❌ sessionToken должен быть hex-строкой.")
    if session_token:
        session_token = session_token.lower()

    # 1. sessionToken: аргумент -> кэш -> логин
    if not session_token and not a.relogin and os.path.exists(CACHE):
        try:
    ...
    if not session_token:
        email = email or input("Email Mozilla-аккаунта: ").strip()
        pw = a.password or getpass.getpass("Пароль: ")
        print("🔑 Вход в Mozilla Accounts...")
        session_token = fxa_login(email, pw, a.totp)
        save_cache(email, session_token)
        print(f"💾 sessionToken закэширован: {CACHE}")
    else:
        print("🔑 Используется переданный sessionToken (шаг /account/login пропущен).")
        if email:
            save_cache(email, session_token)

    # 2-4. OAuth -> Guardian -> proxyPass
    ...
    locations, recommended = fetch_serverlist(a.firefox_version,
                                              a.client_country,
                                              a.include_locked)
    if a.country:
        locations = [l for l in locations
                     if a.country.lower() in (l["countryCode"].lower(), l["countryName"].lower())]
    for l in locations:
        if a.city:
            l["cities"] = [c for c in l["cities"]
                           if a.city.lower() in ((c["cityName"] or "").lower())]
    locations = [l for l in locations if l["cities"]]
    ...
    if a.no_save:
        ...
        out = a.json or os.path.join(SCRIPT_DIR, "mozvpn-" + time.strftime("%Y%m%d-%H%M%S", time.gmtime(now)) + ".json")
